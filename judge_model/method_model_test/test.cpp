...
void Sample_Test::add_vals(std::string s)
{
	...
	{
		std::string string_pair = s.substr(7, s.length() - 7);
		int loc = 0;
		while (string_pair[loc] != ',')
			loc++;
	    int num1 = std::atoi(string_pair.substr(0, loc).c_str());
		int num2=  std::atoi(string_pair.substr(loc+1, string_pair.length()-loc-1).c_str());
		Val temp(s.substr(0, 6), num1, num2);
		vals.push_back(temp);
	}
}
