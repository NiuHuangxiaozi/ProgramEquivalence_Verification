...
void Judge::create_environment()
{
    ...
    for(int i=0;i<fordernames.size();i++)
    {
        ...
        mkdir("./temp/task",S_IRWXU);
        mkdir("./answer/task",S_IRWXU);
...
std::string Judge::get_tempprefix()
{
	return tempprefix;
}
std::string Judge::get_answerfix()
{
	return answerfix;
}
std::string Judge::get_sourcefix()
{
	return sourcefix;
}
std::vector<std::string> Judge::get_filenames()
{
	return fordernames;
}
bool Judge::get_finished()
{
return finished;
}
